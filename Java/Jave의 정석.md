...
1. [변수(Variable)](https://github.com/MinwungRo/Personal-study/blob/main/Java/ave%EC%9D%98%20%EC%A0%95%EC%84%9D..md#1-%EB%B3%80%EC%88%98variable)
...
* 배열은 접근 시간이 신속하지만, 크기를 변경할 수 없으며 비순차적인 데이터의 추가 및 삭제에 시간이 많이 할애된다는 한계가 있다

* LinkedList는 불연속적으로 존재하는 데이터를 서로 연결한 형태로 이러한 단점을 보완한다

* 데이터 삭제시 삭제하고자하는 요소의 이전 요소가 삭제하고자 하는 요소의 다음 요소를 참조하도록 변경한다

* 데이터 추가시 추가 위치 이전 요소의 참조를 새로운 요소로 변경, 새로운 요소가 그 다음요소를 참조하도록 변경한다
...
* 메서드에 선언된 지네릭 타입은 지역 변수를 선언한 것과 같다고 생각하면 된다, 메서드 내에서만 지역적으로 사용될 것이므로 메서드가 static 이건 아니건 무관하다
...
*  자바를 개발한 사람들은 소스코드에 대한 문서를 별도로 생성하기보다 소스코드와 문서를 하나의 파일로 관리하는 것이 효율적이라 생각했다

*  그래서 소스코드의 주석 '/** ~ */'에 소스코드에 대한 정보를 저장하였다

*  '/**'로 시작하는 주석 안에 소스코드에 대한 설명이 있고, 그 내부에 '@'이 붙은 태그들을 이용해서 주석 안에 정보를 저장한다

* 에너테이션은 주석(comment)처럼 프로그래밍 언어에 영향을 미치지 않으면서 다른 프로그램에게 유용한 정보를 제공할 수 있다

* 예를 들어 자신이 작성한 소스코드 중에서 특정 메서드만 테스트하기를 원한다면, 다음과 같이 '@Test'라는 에너테이션을 메서드 앞에 붙인다

```java
@Test	// '이 메서드를 테스트해야 한다'를 알리는 역할 뿐, 기능적인 영향은 미치지 않는다
public void method() { }
...
* 메타 에너테이션은 에너테이션을 정의하는데 사용되는 에너테이션의 에너테이션이다
...
|@Docuneted*|에니테이션 정보가 javadoc으로 작성된 문서에 포함되게 한다|
|@Inherited*|에너테이션이 자손 클래스에 상속되도록 한다|
|@Retention*|애네테이션이 유지되는 범위를 지정하는데 사용한다|
...
* 하기의 코드에서는 오버라이딩할 때 조상 메서드의 이름을 잘못 써도 컴파일러는 이것이 잘못된 것인지 알지 못한다
...
* 그러나 하기의 코드와 같이 '@Override'를 붙이면, 컴파일러가 같은 이름의 메서드가 조상에 있는지 확인하고, 없으면 에러메시지를 출력한다
...
```
* 하기 예시를 컴파일하면 에러메세지가 나타난다, '@Override'를 메서드 앞에 붙이지 않았다면 나타나지 않았을 메세지이다
...
* 새로운 버젼의 JDK 소개될 때, 새로운 기능이 추가될 뿐 아니라 기존의 부족했던 기능을 개선하기도 한다

* 이 과정에서 기존의 기능을 대체할 것들이 추가되어도, 이미 여러 곳에서 사용되고 있을지 모르는 기존의 것들을 함부로 삭제할 수 없다

* 그래서 고안한 방안이 더 이상 사용되지 않는 필드나 메서드에 '@Deprecated'를 붙이는 것이다

* 만일 '@Deprecated'가 붙은 대상을 사용하는 코드를 작성하면, 컴파일할 때 에러 메세지가 나타난다
...
* 필수는 아니지만, 붙이면 실수를 방지할 수 있으므로 '함수형 인터페이스'를 선언할 대는 이 에너지테이션을 붙이는 것이 적절하다
...
* 이전 예시처럼 컴파일러의 경고메세지는 무시하고 넘어갈 수도 있으나, 모두 확인하고 해결해서 컴파일 후에 어떠한 메세지도 나타나지 않게 해야 한다

* 그러나 경우에 따라서 경고가 발생할 것을 알면서도 묵인해야 할 때가 있는데, 묵인해야 하는 경고가 발생하는 대상에 반드시 '@SuppressWarnings'를 붙여서 컴파일 후에 어떤 경고 메세지도 나타나지 않게 해야 한다

* '@SuppressWarnings'로 억제할 수 있는 경고 메세지의 종류는 여러 가지가 있는데, 이 중에서 주로 사용되는 것은 'deprecation', 'unchecked', 'rwatpyes', 'varargs'이다
...
```
* 만일 둘 이 상의 경고를 동시에 억제하려면 다음과 같이 해야 한다, 배열에서처럼 괄호{}를 추가로 사용해야 한다는 것에 주의해야 한다
...
* '애너테이션을 위한 애너테이션', 애너테이션의 적용대상(target)이나 유지기간(retention)등을 지정하는데 사용된다
